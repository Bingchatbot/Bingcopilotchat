    ...
    <div className={cn('thread', { active: isEdit })}>
      <div className="primary-row flex w-full">
        ...
        <div className="controls">
          {!isEdit ? (<>
            <button className="edit icon-button" type="button" aria-label="Изменить" onClick={() => setEdit(true)}>
              <IconEdit />
            </button>

            <button className="delete icon-button" type="button" aria-label="Удалить" onClick={handleDelete}>
              <IconTrash />
            </button>

            <button className="export icon-button" type="button" aria-label="Скачать" onClick={handleDownload}>
              <IconDownload />
            </button>
          </>) : (
            <>
              <button className="edit icon-button" type="button" aria-label="Сохранить" onClick={handleSave}>
                <IconCheck />
              </button>
              <button className="edit icon-button" type="button" aria-label="Закрыть" onClick={() => setEdit(false)}>
                <IconClose />
              </button>
            </>
          )}
