...
export function AdvancedSetting() {
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Дополнительные настройки</DialogTitle>
        <DialogDescription>
          Добавьте несколько полезных функций в New Bing.
        </DialogDescription>
      </DialogHeader>

      <div className="flex flex-col gap-2">
        Отключить лимит разговоров
        <Switch
                        ...
                        <div className="flex gap-2 w-full items-center justify-between">
                          <div className="flex flex-1 items-center">
                            <div className="text-sm w-full">
                              ...
                              >
                                {checked && prompt.label === 'настроить' ?
                                <Textarea placeholder="Оставьте это поле пустым, чтобы увидеть оригинальную версию New Bing" onChange={(event) => handleChangePrompt(event.target.value)} value={systemPrompt || prompt.content} /> : <span>{prompt.desc}</span>
                                }
                              </RadioGroup.Description>
                            </div>
                          </div>
