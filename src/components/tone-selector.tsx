...
const ToneList: ToneItem[] = [
  { name: 'Более креативный', type: BingConversationStyle.Creative },
  { name: 'Более сбалансированный', type: BingConversationStyle.Balanced },
  { name: 'Более точный', type: BingConversationStyle.Precise }
]
            ...
            ToneList.map(tone => (
              <li className="option" key={tone.name} onClick={() => onChange?.(tone.type)}>
                <button className={cn(`tone-${type.toLowerCase()}`, { selected: tone.type === type}) } aria-pressed="true" >
                  <span className="caption-2-strong label-modifier">Обновить</span>
